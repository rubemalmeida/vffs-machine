...
def main():
    ...
    substitute_code_in_file('temp-model.html', '<script type="text/javascript" src="model.js"></script>', f'<script>{js_code}</script>')
    substitute_code_in_file('temp-model.html', '<link rel="stylesheet" href="model.css">', f'<style>{css_code}</style>')

    # Leitura do arquivo temp-model.html
    with open('temp-model.html', 'r') as html_file:
        new_html = html_file.read()

    new_ino = read_file_content('vffs-webserver.ino', 'rawliteral(%HTML%)rawliteral', f'rawliteral({new_html})rawliteral')
