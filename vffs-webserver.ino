...
#define PIN_LED_BRANCO                                                         \
  5 // caso o led esteja aceso, indica que a máquina está ligada e conectada ao
    // Wi-Fim, e caso esteja piscando, indica que a máquina está ligada, porém,
    // não está conectada ao Wi-Fi
#define PIN_VALVULA_LIBERACAO                                                  \
  18 // RL4IN1 - LIBERACAO - válvula pneumatica para controle do fluxo de ar
#define PIN_VALVULA_ENVASE                                                     \
  19 // RL4IN2 - ENVASE - válvula pneumatica para controle do fluxo do fluido a
     // ser envasado
#define PIN_VALVULA_SELAGEM                                                    \
  21 // RL4IN3 - SELAGEM - válvula pneumatica para controle do pistão de selagem
#define PIN_MOTOR_BOMBA                                                        \
  22 // RL4IN4 - BOMBA - motor para controle transporte do fluido a ser envasado
#define PIN_MEDIDOR_FLUXO                                                      \
  23 // medidor de fluxo para controle do volume de fluido a ser envasado
...
const int INTERVALO_MINIMO =
    3000; // intervalo mínimo entre acionamentos do botão (em milisegundos)
...
unsigned long btnProcStartAcionamento =
    0; // Adicionado para rastrear o tempo de pressionamento do botão
unsigned long btnProcTempoUltimoAcionamento =
    0; // tempo em milisegundos do último acionamento do botão
...
unsigned long btnCalibTempoUltimoAcionamento =
    0; // tempo em milisegundos do último acionamento do botão
unsigned long btnCalibTempoPrimeiroPressionado = 0;

unsigned long inicioSelagem = 0; // tempo em milisegundos do início da selagem

unsigned long btnParadaTempoUltimoAcionamento =
    0; // tempo em milisegundos do último acionamento do botão
unsigned long btnParadaStartAcionamento =
    0; // Adicionado para rastrear o tempo de pressionamento do botão
bool paradaEmergencial =
    false; // Flag para indicar se a parada emergencial foi acionada

unsigned long ultimaNotificaoConexao =
    0; // tempo em milisegundos da última notificação de conexão
int tempoNotificacaoConexao =
    1; // intervalo de tempo em milisegundos para notificação de conexão
...
bool statusLedBranco =
    false; // Flag para indicar se o led branco está aceso ou apagado
bool statusLedVerde =
    false; // Flag para indicar se o led verde está aceso ou apagado
...
volatile long qtdPulsoCalibracao =
    0; // Contador de pulsos do medidor de fluxo usado na calibração
volatile long qtdPulsoPorSegundo =
    0; // Contador de pulsos do medidor de fluxo por segundo
...
String processor(const String &var) {
  ...
  if (var == "HIDDEN-FORM-MEDICAO") // && qtdPulsoCalibracao > 0 &&
...
void checkWifi() {
  ...
  if (tempStatusWifi != WL_CONNECTED) {
    ...
    if ((millis() - ultimaNotificaoConexao) > tempoNotificacaoConexao) {
      ...
      tempoNotificacaoConexao =
          tempoNotificacaoConexao == 1 ? 5000 : tempoNotificacaoConexao * 3;
      ultimaNotificaoConexao = millis();
      if (!primeiraConexao)
        log("WiFi desconectado, tentando estabelecer conexão... (" +
            String(tempoNotificacaoConexao / 1000) + "s)");
    }
    ...
    log("WiFi conectado, endereço IP: " +
...
void checkCalibracao() {
  ...
  if (btnCalibUltimoEstado == HIGH && btnCalibEstadoCorrente == LOW &&
  ...
  } else if ((btnCalibUltimoEstado == LOW && btnCalibEstadoCorrente == HIGH &&
              btnCalibUltimoEstadoEfetivo == HIGH) ||
...
void checkEnvase() {
  ...
  if (paradaEmergencial ==
          false && // Se a parada emergencial não estiver acionada
      !modoEnvase &&
      !modoCalibracao && !modoSelagem && // Se não estiver em nenhum modo
      btnProcUltimoEstado == HIGH && btnProcEstadoCorrente == LOW &&
  ...
  } else if ((btnProcUltimoEstado == LOW && btnProcEstadoCorrente == HIGH &&
              btnProcUltimoEstadoEfetivo == HIGH) ||
...
void checkSelagem() {
  if (modoSelagem) {
    ...
    } else if ((millis() - inicioSelagem >= tempoSelagem * 1000 ||
                paradaEmergencial)) {
      if (paradaEmergencial)
        paradaEmergencial = false;
...
void checkParadaEmergencial() {
  ...
  if (buttonState == LOW) {
    if ((millis() - btnParadaStartAcionamento) >
        100) // Se o botão foi mantido pressionado por 500ms
    {
      btnParadaStartAcionamento =
          millis(); // Registre o tempo de pressionamento do botão
      if ((millis() - btnParadaTempoUltimoAcionamento) >= INTERVALO_MINIMO) {
        ...
        // {
        //     if (i == PIN_BOTAO_PROCESSO || i == PIN_BOTAO_CALIBRACAO || i ==
        //     PIN_BOTAO_EMERGENCIAL || i == PIN_MEDIDOR_FLUXO)
        //         continue;
...
void checkMedicao() {
  if (((modoEnvase || modoCalibracao) &&
       (millis() - medicaoPreviousMillis >= intervalMedicao)) ||
      (!modoEnvase && pulse1Sec > 0)) {
    if ((modoEnvase && qtdPulsoAMonitorar > 0) ||
        (!modoEnvase && pulse1Sec > 0)) {
      ...
      // flowRate = ((1000.0 / (millis() - medicaoPreviousMillis)) * pulse1Sec);
      // // in L/min
      flowRate = 1;
      ...
      log("Envasando (" + String(tempoDecorrido / 1000) + "s)... " +
          String(qtdPulsoAtual) + " pulsos somados");
    } else if (modoCalibracao) {
      int tempoDecorrido = millis() - btnCalibTempoPrimeiroPressionado;
      log("Calibrando (" + String(tempoDecorrido / 1000) + "s)... " +
          String(GET_PULSE_COUNT()) + " pulsos somados");
    }
...
void salvar(AsyncWebServerRequest *request) {
  ...
  for (int i = 0; i < params; i++) {
    ...
    //         log("Volume dispensado parametrizado: " +
    //         String(volumeDispensado)); fatorCalibracao = (volumeDispensado /
    //         qtdPulsoCalibracao) * 1000000; log("Fator de calibracao
    //         calculado: " + String(fatorCalibracao));
    //     }
    //     else
    //     {
    //         log("Calibracao não identificada. Qtd de pulsos: " +
    //         String(qtdPulsoCalibracao)); erro = true;
    //     }
    // }
    ...
    // }
      ...
      // qtdPulsoAMonitorar = round((volumeAserEnvasado * 1000000) /
      // fatorCalibracao);
      log("Quantidade de pulsos a monitorar: " + String(qtdPulsoAMonitorar) +
          " (" + tipoLiquido + ")");
      // }
      // else
      // {
      ...
      if (valor == LIQ_AGUA || valor == LIQ_MARACUJA || valor == LIQ_MORANGO ||
          valor == LIQ_ABACAXI || valor == LIQ_MANGA) {
        tipoLiquido = valor;
...
void envase(bool btnPressionado) {
  if (modoEnvase == true) {
    long qtdPulsoAtual = GET_PULSE_COUNT();
    if (!btnPressionado && (totalMilliLitres > volumeAserEnvasado ||
                            qtdPulsoAtual >= qtdPulsoAMonitorar)) {
      PIN_CONTROL(PIN_VALVULA_ENVASE, TURN_OFF);
      PIN_CONTROL(PIN_MOTOR_BOMBA, TURN_OFF);
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_MEDIDOR_FLUXO), contadorPulso,
                  RISING);
  ...
  server.on("/parametros", HTTP_GET, [](AsyncWebServerRequest *request) {
    String response = String(qtdPulsoCalibracao) + "|" + tipoLiquido + "|" +
                      tempoSelagem + "|" + volumeAserEnvasado;
  ...
  server.on("/medicao", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    eventos = "";
    String atividadeVigente =
        modoEnvase ? "ENVASE" : (modoCalibracao ? "CALIBRAÇÃO" : "--");
    String response =
        (atividadeVigente + "|" + String(flowMilliLitres) + "|" +
         String(totalMilliLitres) + "|" + String(qtdEnvaseRealizado) + "|" +
         String(GET_PULSE_COUNT()) + "|" + String(qtdPulsoAMonitorar) + "|" +
         eventosString);
    request->send(200, "text/plain", response);
  });
